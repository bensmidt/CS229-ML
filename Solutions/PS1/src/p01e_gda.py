...
def main():
    ...
    x_train1_int, y_train1_int = util.load_dataset(train1_path, add_intercept=True)
    ...
    x_train2_int, y_train2_int = util.load_dataset(train2_path, add_intercept=True)
    x_val2, y_val2 = util.load_dataset(valid2_path, add_intercept=True)

    # fit first dataset
    Model1 = GDA()
    Model1.fit(x_train1, y_train1)
    train1_pred = Model1.predict(x_val1)
    ...
    # save predictions
    np.savetxt(os.path.join(pred_path, "GDA_val1.txt"), train1_pred)

    # fit second dataset
    Model2 = GDA()
    Model2.fit(x_train2, y_train2)
    train2_pred = Model2.predict(x_val2)
...
class GDA(LinearModel):
    ...
    def fit(self, x, y):
        ...
        # get theta values
        self.theta[0] = 1/2 * (u1 + u0) @ np.linalg.inv(cov) @ (u1 - u0) + np.log(1/phi - 1)
        self.theta[1:] = np.linalg.inv(cov) @ (u0 - u1)
